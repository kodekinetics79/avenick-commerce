...
async function fixture(label: string) {
  ...
  users.push(requester.id, owner.id);
  ...
  const company = await db.company.create({ data: {
    ...
    members: { create: { userId: requester.id, role: "COMPANY_BUYER", isActive: true, spendLimit: null } },
  ...
  return { requester, company, member: company.members[0]!, po };
}
