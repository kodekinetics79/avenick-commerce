      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {quotes.length === 0 ? (
              ...
              quotes.map((q) => {
                ...
                return (
                  <div key={q.id} className={`bg-card rounded-2xl border-2 overflow-hidden transition-shadow hover:shadow-md ${isBest ? "border-primary" : "border-border"}`}>
                    {isBest && (
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex items-start justify-between gap-4 mb-4">
                        <div>
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${q.sellerTier === "GOLD" ? "bg-amber-500/15 text-amber-600 dark:text-amber-400" : "bg-primary/20 text-primary"}`}>
                              {q.sellerTier}
                            </span>
                          </div>
                      ...
                      {/* Details */}
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4 text-sm bg-secondary/30 rounded-xl p-3">
                      ...
                      {q.notes && (
                        <div className="flex items-start gap-2 mb-4 text-sm text-muted-foreground bg-primary/10 border border-primary/20 rounded-xl p-3">
            ...
            {quotes.length > 1 && (
              <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 flex items-center gap-2 text-sm text-foreground">
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-3">
                {TIMELINE.map((step, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${step.done ? "bg-primary" : "bg-secondary"}`}>
                      {step.done
            ...
            {/* Quick links */}
            <div className="bg-card rounded-2xl border border-border p-4">
              <p className="text-sm font-semibold mb-2 text-foreground">Related</p>
            ...
            <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 text-sm text-foreground">
              <p className="font-semibold text-amber-600 dark:text-amber-400 mb-1">Need help?</p>
              <p className="text-muted-foreground mb-3">Your account manager Sara Al-Ahmed is available to assist with this RFQ.</p>
