...
export async function createPolicy(_prev: B2BActionState, formData: FormData): Promise<B2BActionState> {
  ...
  if (!ctx || ctx.member.role !== "COMPANY_ADMIN") return { error: t("act.policies.adminOnly") };
  ...
  if (!name) return { error: t("act.policies.needName") };
  if (!threshold || Number.isNaN(threshold) || threshold <= 0) return { error: t("act.policies.needThreshold") };
  ...
  return { ok: true, message: t("act.policies.added", { name }) };
}
