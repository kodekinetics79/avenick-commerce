    ...
    <MainLayout>
      <div className="bg-slate-50 min-h-screen">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          {/* Tabs */}
          <div className="flex gap-1 bg-white border border-border rounded-xl p-1 mb-5 w-fit">
            {TABS.map((tab) => (
              <button key={tab} type="button"
                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${tab === "All" ? "bg-slate-900 text-white" : "text-muted-foreground hover:text-foreground hover:bg-slate-50"}`}>
                {tab}
          ...
          <div className="space-y-4">
            {MOCK_APPROVALS.map((approval) => {
              ...
              return (
                <div key={approval.id} className={`bg-white rounded-2xl border-2 overflow-hidden ${approval.status === "PENDING" ? uc.border : "border-border"}`}>
                  {/* Top bar */}
                  <div className={`px-5 py-2.5 flex items-center justify-between border-b ${approval.status === "PENDING" && approval.urgency !== "normal" ? `bg-${approval.urgency === "critical" ? "red" : "amber"}-50 border-${approval.urgency === "critical" ? "red" : "amber"}-100` : "bg-slate-50 border-border"}`}>
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold mb-1">{approval.description}</h3>
                        ...
                        {approval.notes && (
                          <p className="text-xs text-muted-foreground bg-slate-50 rounded-lg px-3 py-2 border border-border">{approval.notes}</p>
                        )}
                      </div>
