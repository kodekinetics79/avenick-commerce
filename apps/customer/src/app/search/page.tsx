        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="text-xl font-bold">
              {query ? <>Results for <em className="not-italic text-orange-600">&ldquo;{query}&rdquo;</em></> : "Search Products"}
            </h1>
          </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {POPULAR_SEARCHES.map((s) => (
                    <Link key={s} href={`/search?q=${encodeURIComponent(s)}`}
                      className="px-3 py-1.5 bg-white border border-border rounded-full text-sm hover:border-orange-400 hover:text-orange-600 transition-colors">
                      {s}
                    </Link>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {POPULAR_CATEGORIES.map((cat) => (
                    <Link key={cat.slug} href={`/products?category=${cat.slug}`}
                      className="bg-white rounded-2xl border border-border p-4 text-sm font-medium text-center hover:border-orange-400 hover:shadow-sm transition-all">
                      {cat.name}
                    </Link>
                  ))}
                </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-border text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-6">
                {POPULAR_SEARCHES.slice(0, 4).map((s) => (
                  <Link key={s} href={`/search?q=${encodeURIComponent(s)}`}
                    className="px-3 py-1.5 bg-orange-50 text-orange-600 border border-orange-200 rounded-full text-sm hover:bg-orange-100 transition-colors">
                    {s}
                  </Link>
                ))}
              </div>
              <Link href="/products" className="text-sm text-blue-600 hover:underline">Browse all products →</Link>
            </div>
          )}
