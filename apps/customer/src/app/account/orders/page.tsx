        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          {orders.length === 0 ? (
            <div className="bg-white rounded-2xl border border-border p-16 text-center">
              ...
              <Link href="/products" className="inline-flex items-center gap-2 bg-green-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-green-700 transition-colors">
                Start Shopping →
              </Link>
            </div>
          ) : (
              ...
              {orders.map((order) => {
                ...
                return (
                  <Link key={order.id} href={`/orders/${order.id}`}
                    className="block bg-white rounded-2xl border border-border hover:border-green-300 hover:shadow-sm transition-all group">
                    <div className="p-4">
                      {/* Top row */}
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-green-700">{formatCurrency(Number(order.total), order.currency)}</span>
                        ...
                        {order.items.map((item, i) => (
                          <div key={i} className="flex items-center gap-1.5">
                            ...
                            <p className="text-sm text-muted-foreground line-clamp-1">
                              {item.nameEn}
                              {item.quantity > 1 && <span className="ms-1 text-xs font-medium text-slate-500">×{item.quantity}</span>}
                            </p>
                          </div>
                        ))}
                      ...
                      {order.status === "DELIVERED" && (
                        <div className="mt-2 flex items-center gap-1.5 text-xs text-muted-foreground">
                          ...
                          <span>Delivered · <Link href="/returns" className="text-green-600 hover:underline">Return or exchange</Link></span>
                        </div>
                      )}
                    </div>
                  </Link>
