...
export default function DealsPage() {
  ...
  return (
    <MainLayout>
      {/* Hero Banner */}
      <section className="relative bg-gradient-to-br from-red-700 via-red-600 to-primary/100 text-white overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(255,255,255,0.1),transparent_60%)]" />
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-primary/20 rounded-full blur-3xl pointer-events-none" />
        <div className="relative max-w-7xl mx-auto px-4 py-14 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="text-primary/20 text-lg mb-3">Save up to 25% on top products from verified GCC suppliers.</p>
          ...
          <div className="flex items-center gap-4 bg-black/20 border border-white/20 rounded-2xl px-8 py-5 backdrop-blur-sm shrink-0">
            ...
            <div>
              <p className="text-xs text-primary/30 uppercase tracking-widest mb-1">Offers end in</p>
              <p className="text-4xl font-mono font-bold tracking-wider">14:22:08</p>
              <p className="text-xs text-primary/40 mt-1">Prices reset daily at midnight</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="flex items-center gap-3 mb-6 overflow-x-auto pb-2">
          ...
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
              type="button"
              className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${cat === "All" ? "bg-primary/100 text-white" : "bg-white border border-border hover:border-primary/40 text-muted-foreground hover:text-primary"}`}
            >
              {cat}
            </button>
          ))}
          ...
          {dealsProducts.map((product) => (
            <div key={product.id} className="bg-white rounded-2xl border border-border overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all group">
              ...
              <div className="p-3.5">
                ...
                <Link href={`/products/${product.slug}`}>
                  <h3 className="text-sm font-semibold line-clamp-2 mb-2.5 hover:text-primary transition-colors leading-snug">{product.nameEn}</h3>
                </Link>
