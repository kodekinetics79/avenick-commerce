      ...
      <div className="bg-slate-50 min-h-screen">
        <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          <div className="bg-white rounded-2xl border border-border p-5 mb-5">
            <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
              ...
              <div className="text-end">
                <p className="text-2xl font-bold text-green-700">AED {order.total.toFixed(2)}</p>
          ...
          {isShipped && (
            <div className="bg-white rounded-2xl border border-border p-5 mb-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button type="button" onClick={() => setShowTracking(v => !v)} className="text-xs text-blue-600 hover:underline font-medium">
              ...
              <div className="flex items-center justify-between bg-slate-50 rounded-xl px-4 py-3 mb-3">
                <div>
                  ...
                  <p className="font-mono text-sm font-semibold text-slate-800">{MOCK_TRACKING.trackingNumber}</p>
                </div>
                  ...
                  {MOCK_TRACKING.events.map((ev, i) => (
                    <div key={i} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div className={`h-6 w-6 rounded-full flex items-center justify-center shrink-0 ${i === 0 ? "bg-green-500" : "bg-slate-200"}`}>
                      ...
                      <div className="pb-2">
                        <p className={`text-sm font-medium ${i === 0 ? "text-green-700" : "text-slate-600"}`}>{ev.desc}</p>
              ...
              {ALL_STEPS.map((step, idx) => {
                ...
                return (
                  <div key={step.status} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`h-9 w-9 rounded-full flex items-center justify-center shrink-0 ${isReached ? "bg-green-500 shadow-sm shadow-green-200" : isCurrent ? "bg-blue-500 ring-4 ring-blue-100" : "bg-slate-100"}`}>
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          <p className={`font-semibold text-sm ${isFuture ? "text-muted-foreground" : ""}`}>
                            {step.label}
                            {isCurrent && <span className="ms-2 text-[10px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded-full font-bold uppercase">Current</span>}
                          </p>
                          <p className={`text-xs mt-0.5 ${isFuture ? "text-slate-300" : "text-muted-foreground"}`}>
                            {entry ? entry.date : isFuture ? "Upcoming" : step.desc}
                          </p>
                        </div>
                        {isReached && <CheckCircle className="h-4 w-4 text-green-500 shrink-0 mt-0.5" />}
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="divide-y divide-border">
              {order.items.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between px-5 py-3.5">
                  ...
                  <p className="font-bold text-sm text-green-700">AED {(item.quantity * item.unitPrice).toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl border border-border p-5">
              ...
              <div className="space-y-1.5 text-sm">
                ...
                <div className="flex justify-between font-bold text-base pt-2 border-t border-border">
                  <span>Total</span><span className="text-green-700">AED {order.total.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {isDelivered && (
              <Link href="/returns" className="flex-1 flex items-center justify-center gap-2 bg-white border border-border text-slate-700 hover:border-orange-300 hover:text-orange-600 font-medium px-4 py-2.5 rounded-xl text-sm transition-colors">
