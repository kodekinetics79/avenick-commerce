...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof CheckCircle }> = {
  OPEN:  { label: "Open",  color: "bg-amber-500/10 text-amber-700 dark:text-amber-400", icon: Clock },
  FILED: { label: "Filed", color: "bg-green-500/10 text-green-700 dark:text-green-400", icon: CheckCircle },
};
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Net VAT Due (Open)", value: formatCurrency(totalDue, "AED"), color: "text-amber-600", bg: "bg-amber-500/10 border-amber-500/20" },
            { label: "Output VAT (Collected)", value: formatCurrency(totalOutput, "AED"), color: "text-green-700", bg: "bg-card border-border" },
            { label: "Input VAT (Reclaimable)", value: formatCurrency(totalInput, "AED"), color: "text-primary", bg: "bg-card border-border" },
            { label: "Open Periods", value: openPeriods.length, color: "text-foreground", bg: "bg-card border-border" },
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded font-mono">{p.country}</span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {p.status === "OPEN"
                          ? <button type="button" className="text-xs bg-primary text-primary-foreground px-2.5 py-1 rounded-lg hover:bg-primary/90 font-medium transition-colors">File Return</button>
