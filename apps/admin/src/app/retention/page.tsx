...
const RISK_CONFIG: Record<string, { label: string; color: string; border: string; dot: string }> = {
  HIGH:   { label: "High Risk",   color: "bg-red-500/10 text-red-700 dark:text-red-400",     border: "border-red-500/20",   dot: "bg-red-500" },
  MEDIUM: { label: "Medium Risk", color: "bg-amber-500/10 text-amber-700 dark:text-amber-400", border: "border-amber-500/20", dot: "bg-amber-500" },
  LOW:    { label: "Low Risk",    color: "bg-primary/10 text-primary",   border: "border-primary/20",  dot: "bg-blue-500" },
};

const STAGE_COLORS: Record<string, string> = {
  slate: "text-muted-foreground bg-muted", blue: "text-primary bg-primary/10", green: "text-green-600 bg-green-500/10",
  amber: "text-amber-600 bg-amber-500/10", red: "text-red-600 bg-red-500/10",
};
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <Link href="/campaigns" className="flex items-center gap-1.5 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-semibold px-4 py-2 rounded-xl transition-colors">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Retention Rate", value: `${retentionRate}%`, color: "text-green-600", bg: "bg-green-500/10 border-green-500/20" },
            { label: "High-Risk Accounts", value: highRisk.length, color: "text-red-600", bg: "bg-red-500/10 border-red-500/20" },
            { label: "At-Risk Value", value: formatCurrency(atRiskValue, "AED"), color: "text-amber-600", bg: "bg-amber-500/10 border-amber-500/20" },
            { label: "Churned (90d)", value: churnedCount, color: "text-muted-foreground", bg: "bg-card border-border" },
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${r.type === "B2B" ? "bg-purple-500/10 text-purple-700 dark:text-purple-400" : "bg-primary/10 text-primary"}`}>{r.type}</span>
                      </div>
                    ...
                    <div className="text-end shrink-0">
                      ...
                      <div className="flex flex-col gap-1.5">
                        <button type="button" className="flex items-center gap-1 text-xs bg-primary text-primary-foreground px-3 py-1.5 rounded-lg hover:bg-primary/90 font-medium transition-colors justify-center">
                          <Phone className="h-3 w-3" /> Call
                        </button>
                        <button type="button" className="flex items-center gap-1 text-xs border border-border text-muted-foreground px-3 py-1.5 rounded-lg hover:bg-muted/30 font-medium transition-colors justify-center">
