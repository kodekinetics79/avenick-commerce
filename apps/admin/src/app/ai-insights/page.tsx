  ...
  Commerce: [
    {
      ...
      href: "/products",
      tag: "Revenue",
      tagColor: "bg-primary/10 text-primary dark:text-primary"
    },
    {
      icon: BarChart2,
...
const RECOMMENDED_ACTIONS = [
  { priority: "P1", label: "Invite backup suppliers for 3 unanswered RFQs", color: "bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20", href: "/rfqs" },
  { priority: "P1", label: "Renew expiring compliance docs before 14-day deadline", color: "bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20", href: "/compliance" },
  { priority: "P2", label: "Run reactivation campaign for 5 inactive VIP B2B accounts", color: "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20", href: "/crm" },
  { priority: "P2", label: "Replenish SKU-PPE-0042 — 3 days stock remaining", color: "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20", href: "/warehouse" },
  { priority: "P3", label: "Adjust pricing on 3 overpriced SKUs to improve conversion", color: "bg-primary/10 text-primary dark:text-primary border-primary/20", href: "/products" },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {(Object.keys(INSIGHTS) as Array<keyof typeof INSIGHTS>).map((tab) => (
              <div key={tab}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{INSIGHTS[tab].length} signals</span>
                </div>
                <div className="space-y-3">
                  {INSIGHTS[tab].map((insight, idx) => {
                    const Icon = insight.icon;
                    return (
                      <div key={idx} className="bg-card rounded-2xl border border-border p-4 hover:shadow-sm transition-shadow">
                              ...
                              <div className="flex items-center gap-2 flex-1">
                                ...
                                <div className="flex gap-0.5 flex-1 max-w-[120px] h-1.5">
                                  {Array.from({ length: 10 }).map((_, i) => (
                                    <div key={i} className={cn("flex-1 rounded-full", i < Math.round(insight.confidence / 10) ? (insight.confidence >= 90 ? "bg-green-500" : insight.confidence >= 75 ? "bg-amber-500" : "bg-blue-500") : "bg-muted")} />
                                  ))}
                                </div>
                                <span className={cn("text-xs font-semibold", insight.confidence >= 90 ? "text-green-600 dark:text-green-400" : insight.confidence >= 75 ? "text-amber-600 dark:text-amber-400" : "text-primary")}>
                                  {insight.confidence}%
                                </span>
                              </div>
                              <Link
          ...
          <div className="space-y-4">
            <div className="bg-card rounded-2xl border border-border overflow-hidden sticky top-4">
              <div className="px-5 py-4 border-b border-border bg-gradient-to-r from-slate-800 to-slate-700">
              ...
              <div className="divide-y divide-border">
                {RECOMMENDED_ACTIONS.map((action, idx) => (
                  <Link key={idx} href={action.href} className="flex items-start gap-3 px-4 py-3.5 hover:bg-muted/30 transition-colors group">
