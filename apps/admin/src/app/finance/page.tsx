      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-2">
            <Link href="/payments" className="flex items-center gap-1.5 text-sm border border-border bg-card text-muted-foreground hover:bg-muted/30 px-3 py-2 rounded-xl font-medium transition-colors">
              <CreditCard className="h-3.5 w-3.5" /> Payments
            </Link>
            <Link href="/settlements" className="flex items-center gap-1.5 text-sm bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-2 rounded-xl font-semibold transition-colors">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <Link href="/vat" className="bg-card border border-border rounded-2xl p-4 hover:shadow-sm transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {/* Credit overdue */}
          <div className={`rounded-2xl p-4 flex items-center justify-between border ${creditOverdue > 0 ? "bg-red-500/10 border-red-500/20" : "bg-card border-border"}`}>
            <div className="flex items-center gap-3">
                ...
                {MOCK_FINANCE_INVOICES.map((inv) => (
                  <tr key={inv.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${inv.type === "B2B" ? "bg-purple-500/10 text-purple-700 dark:text-purple-400" : "bg-primary/10 text-primary"}`}>{inv.type}</span>
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${inv.status === "COLLECTED" ? "bg-green-500/10 text-green-700 dark:text-green-400" : "bg-amber-500/10 text-amber-700 dark:text-amber-400"}`}>{inv.status}</span>
                    </td>
