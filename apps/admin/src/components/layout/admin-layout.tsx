...
import {
  LayoutDashboard, Brain, Bell as BellAlert, Package, Tag, Award, Percent,
  Building2, FileQuestion, Quote, CheckSquare, Store, Clock, FileCheck, TrendingUp, Truck,
...
export function AdminLayout({ children, pendingCount = 0 }: { children: React.ReactNode; pendingCount?: number }) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-[#0F172A]">
      {/* Logo area */}
      <div className={cn("px-4 py-4 border-b border-slate-800 flex items-center gap-3", collapsed && "justify-center px-2")}>
        ...
        {!collapsed && (
          <div className="min-w-0">
            <p className="text-white font-bold text-sm leading-tight truncate">Avenick Commerce</p>
            <p className="text-blue-300/80 text-[11px] font-medium tracking-wide">Modern Trade OS</p>
          </div>
        )}
      </div>
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "bg-blue-600 text-white border-l-2 border-blue-400 pl-[9px]"
                  ...
                  >
                    ...
                    {!collapsed && badgeNum > 0 && (
                      <span className="ms-auto bg-red-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center font-bold">
      ...
      <div className="border-t border-slate-800 p-3">
        ...
        {!collapsed && (
          <p className="px-2.5 pt-2 text-[10px] text-slate-600 leading-tight">B2B-first. B2C-ready.<br />Built for modern trade.</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top header */}
        <header className="bg-white border-b border-slate-200 h-14 px-4 flex items-center justify-between shrink-0 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-semibold">STAGING</span>

            {/* Notification bell */}
            <button type="button" className="relative p-2 hover:bg-slate-100 rounded-lg transition-colors">
            ...
            <div className="relative group">
              <button type="button" className="flex items-center gap-2 p-1.5 hover:bg-slate-100 rounded-lg transition-colors">
                <div className="h-7 w-7 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-xs">A</div>
              ...
              <div className="absolute end-0 top-full mt-1 w-44 bg-white border border-slate-200 rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                <div className="px-4 py-3 border-b border-slate-100">
                  ...
                  <p className="text-xs text-slate-400">admin@avenick.com</p>
                </div>
